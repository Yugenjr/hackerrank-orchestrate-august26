...
class PolicyEngine:
    ...
    def evaluate(self, msg_payload: str, context: Dict[str, Any]) -> Optional[Tuple[str, str, float, str]]:
        """
        Returns (action, message_type, confidence, reason) if a deterministic policy overrides the LLM.
        Otherwise returns None.
        """
        overrides = self.policies.get("deterministic_overrides", {})
        
        # Sort by priority (highest first)
        sorted_rules = sorted(overrides.items(), key=lambda x: x[1].get("priority", 0), reverse=True)
        ...
        for rule_name, rule in sorted_rules:
            ...
            if rule_name == "forwarded_spam":
                ...
                if fw_count >= rule.get("forward_count_min", 5) and trust <= rule.get("sender_trust_max", 0.2):
                    return (rule["force_action"], rule["force_type"], 0.95, "Deterministic match: Heavily forwarded from low-trust sender.")
